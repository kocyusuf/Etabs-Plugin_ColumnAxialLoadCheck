...
class ColumnSectionCheck(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        ret_comboslist = self.SapModel.RespCombo.GetNameList()
        self.combo_list = [combos for combos in ret_comboslist[1: -1]]
        self.combo_list_list = []
        for i in self.combo_list:
            for j in i:
                self.combo_list_list.append(j)
        print(self.combo_list_list)
        self.ui.combos_box.setFixedWidth(130)
        self.ui.combos_box.addItems(self.combo_list_list)

    def check_columns(self):
        # self.SapModel.SetModelIsLocked(False)
        # self.SapModel.Analyze.RunAnalysis()

        ret_frames = self.SapModel.FrameObj.GetLabelNameList()
        elements = [i for i in ret_frames[2]]
        columns = [id for id in elements if "C" in id]
        columns = set(columns)
        columns = list(columns)
        self.column_labels = sorted(columns)
        self.column_ids = []
        for item in self.column_labels:
            splits = item.split("C")
            self.column_ids.append(splits[1])
        for i in self.column_ids:
            self.ret1 = self.SapModel.Results.Setup.DeselectAllCasesAndCombosForOutput()
            self.ret2 = self.SapModel.Results.Setup.SetComboSelectedForOutput(self.ui.combos_box.currentText())
            self.ObjectElm = 0
